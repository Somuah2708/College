...
export default function GlobalExposureScreen() {
  ...
  const fetchGlobalExposureData = async () => {
    try {
      ...
      const mockJobs: JobOpportunity[] = Array.from({ length: 100 }, (_, index) => {
        const companies = [
          'Google', 'Microsoft', 'Apple', 'Amazon', 'Meta', 'Netflix', 'Tesla', 'Uber', 'Airbnb', 'Spotify',
          'Adobe', 'Salesforce', 'Oracle', 'IBM', 'Intel', 'NVIDIA', 'Cisco', 'VMware', 'Dropbox', 'Slack',
          'Twitter', 'LinkedIn', 'PayPal', 'Square', 'Stripe', 'Zoom', 'Shopify', 'Atlassian', 'GitHub', 'GitLab'
        ];
        
        const jobTitles = [
          'Software Engineer', 'Senior Software Engineer', 'Data Scientist', 'Product Manager', 
          'Engineering Manager', 'Tech Lead', 'Principal Engineer', 'Research Scientist',
          'AI Engineer', 'DevOps Engineer', 'Security Engineer', 'Full Stack Developer'
        ];
        ...
        return {
          ...
          location_city: city,
          job_description: `Join ${company} as a ${jobTitle} and work on innovative projects that impact millions of users globally. You'll collaborate with world-class engineers and contribute to cutting-edge technology solutions.`,
          application_url: `https://${company.toLowerCase().replace(' ', '')}.com/careers`,
          ...
          requirements: 'Bachelor\'s degree in Computer Science or related field, Strong programming skills, Experience with modern frameworks',
          benefits: 'Health insurance, Stock options, Flexible work arrangements, Professional development budget',
          contact_email: `careers@${company.toLowerCase().replace(' ', '')}.com`,
          company_website: `https://${company.toLowerCase().replace(' ', '')}.com`,
          is_remote: isRemote,
      ...
      const mockExchanges: ExchangeProgram[] = Array.from({ length: 50 }, (_, index) => {
        const universities = [
        ...
        return {
          id: `exchange-${index + 1}`,
          program_name: `${programType} at ${university.name}`,
          host_university: university.name,
          host_country: university.country,
          host_city: university.city,
          program_description: `Experience world-class education at ${university.name} through our ${programType}. Immerse yourself in a different academic culture while advancing your studies in computer science.`,
          ...
          eligibility_requirements: 'Minimum GPA 3.5, Completed 2 years of study, Good academic standing',
          ...
          academic_requirements: 'Strong performance in core CS subjects, Letter of recommendation from faculty',
